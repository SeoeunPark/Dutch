...
class Receipt:
    def __init__(self, receipt, inputmenu):
        self.inputm = inputmenu
        self.one = self.inputm[0]


        fontq = tkinter.font.Font(size=13, weight='bold')
        fontm = tkinter.font.Font(size=10, weight='bold')
        self.receipt = receipt

        self.receiptBack = tkinter.PhotoImage(file="image/white.png")
        self.receiptBackL = tkinter.Label(image=self.receiptBack)
        self.receiptBackL.place(x=-2, y=-2)

        self.locationText = Label(self.receipt, text=self.inputm,fg='#ff7878', font=fontm, bg='white')
        ...
        self.saveButton = Button(self.receipt, width=30, text='저 장', repeatdelay=20, bg='#ff7878', font=fontq,
